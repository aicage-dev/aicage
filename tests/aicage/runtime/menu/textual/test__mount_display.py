...
class MountDisplayTests(TestCase):
    ...
    def test_overview_mount_list_items_returns_built_in_and_custom_items(self) -> None:
        values = _mount_display.overview_mount_list_items(
            ...
            [CustomShareValue("/tmp/logs:ro"), CustomShareValue("/tmp/data")],
        )

        self.assertEqual("SSH", values[0].prefix)
        self.assertEqual("/tmp/.ssh", values[0].path)
        ...
        self.assertEqual("/tmp/logs", values[1].path)
        self.assertEqual("custom:/tmp/logs:ro", values[1].selection_key)
        self.assertEqual(None, values[2].prefix)
        self.assertEqual("/tmp/data", values[2].path)
        self.assertEqual("custom:/tmp/data", values[2].selection_key)
    ...
    def test_confirm_mount_list_items_returns_confirm_items(self) -> None:
        ...
        self.assertEqual("Read-only: /tmp/gh", values[0].path)
        self.assertEqual("builtin:extension:gh:/tmp/gh:ro", values[0].selection_key)

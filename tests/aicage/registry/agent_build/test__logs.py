...
class AgentBuildLogsTests(TestCase):
    def test_build_log_path_uses_timestamp(self) -> None:
        with mock.patch("aicage.registry.agent_build._logs.timestamp", return_value="stamp"):
    ...
    def test_build_log_path_uses_base_dir(self) -> None:
        with (
            mock.patch("aicage.registry.agent_build._logs.IMAGE_BUILD_LOG_DIR", Path("/tmp/logs")),
            mock.patch("aicage.registry.agent_build._logs.timestamp", return_value="stamp"),
        ):
            build_log = _logs.build_log_path("claude", "ubuntu")

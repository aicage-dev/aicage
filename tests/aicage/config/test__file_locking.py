...
class FileLockingTests(TestCase):
    def test_lock_project_config_creates_paths_and_uses_portalocker(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            project_path = Path(tmp_dir) / "aicage" / "test" / "project" / "lockfile"
            with mock.patch(
    ...
    def test_lock_project_config_uses_one_lock(self) -> None:
        ...
        with mock.patch(
            ...
            side_effect=[fake_lock(Path("/test-tmp/project"))],
        ) as lock_mock:
            with file_locking._lock_project_config(Path("/test-tmp/project")):
                pass
        self.assertEqual(1, lock_mock.call_count)

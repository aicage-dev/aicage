...
class ShareEditorScreen(CancelableScreen[ShareEditorResult | None]):
    ...
    def compose(self) -> ComposeResult:
        yield Container(
            Container(
                ...
                Static("Edit Bind Mount" if self._allow_remove else "Add Bind Mount", classes="screen_title"),
                Static("Host path", classes="screen_subtitle"),
                Input(value=self._path_value, id="share_input", classes="field bind_mount_input"),
                Horizontal(
                    Checkbox("Read-only", value=self._read_only, id="read_only_toggle", classes="bind_mount_checkbox"),
                    classes="bind_mount_checkbox_row",
                ),
                Static("Browse filesystem", classes="screen_subtitle"),
                Vertical(
                    DirectoryTree(Path("/"), id="bind_mount_tree", classes="bind_mount_tree"),
                    classes="bind_mount_tree_frame",
                ),
                Horizontal(
                    Button("Cancel", id="cancel", variant="default"),
                    *([Button("Remove", id="remove", variant="error")] if self._allow_remove else []),
    ...
    def action_accept(self) -> None:
        ...
        self.dismiss(
            ShareEditorResult(
                compose_mount_value(path, self.query_one("#read_only_toggle", Checkbox).value),
                False,
            )
        )
